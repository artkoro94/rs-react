...
const mockedFetchPokemonById =
  vi.mocked(fetchPokemonById);
...
describe('App', () => {
  beforeEach(() => {
    ...
    mockedFetchPokemonById.mockReset();
  });
  ...
  it('loads pokemons on initial render with empty search term', async () => {
    ...
    expect(
      await screen.findByRole('heading', {
        name: pikachuMock.name,
      })
    ).toBeInTheDocument();

    expect(
      screen.getByText(pikachuMock.description)
    ).toBeInTheDocument();

    expect(
      screen.getByRole('heading', {
        name: bulbasaurMock.name,
      })
    ).toBeInTheDocument();

    expect(
      screen.getByText(bulbasaurMock.description)
    ).toBeInTheDocument();
  });
  ...
  it('loads pokemons using saved search term from localStorage', async () => {
    ...
    expect(
      screen.getByPlaceholderText('pikachu')
    ).toHaveValue('pikachu');
  ...
  it('renders error message when API request fails', async () => {
    mockedFetchPokemons.mockRejectedValueOnce(
      new Error('API error')
    );
    ...
    expect(
      await screen.findByText(
        'Could not load pokemons. Try another name.'
      )
    ).toBeInTheDocument();
  ...
  it('shows loading state while request is pending', () => {
    mockedFetchPokemons.mockReturnValue(
      new Promise(() => {})
    );
  ...
  it('renders pokemon details page', async () => {
  mockedFetchPokemons.mockResolvedValueOnce([]);

  mockedFetchPokemonById.mockResolvedValueOnce(
    pikachuMock
  );
